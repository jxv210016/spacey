...
/// Renders a virtual key code as a short, glyph-style label for a hotkey chord
/// (`Space`, `↩`, `A`, `F3`, …). Kept as a static table rather than going through
/// `UCKeyTranslate` so it is deterministic, layout-independent, and unit-testable.
enum KeyCodeNames {
    /// A display label for `keyCode`, falling back to `Key <n>` for anything unmapped.
